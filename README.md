...
```
$ iamspy scan --manifest iam-manifest.yaml src/pipeline.py
```

Produces a YAML file checked into source control alongside your code:

```yaml
version: "1"
generated_by: "iamspy scan src/pipeline.py"
...
permissions:
  ...
  conditional:
    - permission: bigquery.tables.create
      reason: "only if destination table does not already exist"
...
roles:
  ...
  custom:
    ...
    description: "Minimum permissions for pipeline (generated by iamspy)"
    permissions:
      - bigquery.jobs.create
      - secretmanager.versions.access
      - storage.buckets.get
...
The manifest is the single source of truth for what your service needs. Diff it in CI to catch new permissions before deploy. See [docs/permission-manifest.md](docs/permission-manifest.md) for the full format and `iamspy apply` provisioning.
