...
**Build time (offline, ~$6, ~50 min):** Analyze 8.8M lines of GCP SDK source code, extract REST URIs and docstrings, then use Claude to map 13,529 methods to IAM permissions. Output: `iam_permissions.json`.
...
BUILD TIME (build_pipeline/)
  s01: Discover 129 installed SDK packages → service_registry.json
  s02: Fix iam_prefix via Gemini → service_registry.json
  s03: Introspect SDK classes → method_db.json (4,914 methods, 24,330 signatures)
  ...
  s06: LLM mapping (Config D+) → iam_permissions.json (13,529 entries, 129 services)
...
Measured: SDK introspection takes 13.4s (importing 129 packages). File scanning takes <1ms/file. All expensive work is pre-built as static JSON loaded at startup in ~39ms.

## 6. Static Artifacts

| File | Size | Contents |
|---|---|---|
| `service_registry.json` | 33KB | 129 services, modules, IAM prefixes |
| `method_db.json` | 4.7MB | 4,914 unique methods, 24,330 signatures |
| `iam_permissions.json` | 3MB | 13,529 method→permission mappings, 129 services |
...
build_pipeline/
├── __main__.py          # CLI: python -m build_pipeline add/refresh/stats/run
├── stats.py             # python -m build_pipeline.stats
├── stages/              # s01 (registry) through s07 (validate)
│   └── s06_permission_mapping.py  # LLM mapping + prompts + logging (all in one)
└── extractors/          # gapic.py (REST URIs + docstrings), handwritten.py (tree-sitter)
